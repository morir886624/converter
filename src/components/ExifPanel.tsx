...
export function ExifPanel({ file, extension, canClean, isCleaned, onClean }: Props) {
  ...
  const summaryContent = (
    <span className="flex items-center gap-2 flex-wrap">
      ...
      {exif === 'loading' && (
        <span className="text-xs text-slate-400 dark:text-slate-500 italic">reading…</span>
      )}
    </span>
  );
        ...
        <div className="px-3 pb-3 space-y-1.5 border-t border-slate-100 dark:border-slate-700 pt-2">
          {isCleaned ? (
            <p className="text-xs text-emerald-700 dark:text-emerald-300 flex items-center gap-1.5">
              ...
              The cleaned image contains no EXIF metadata.
            </p>
          ) : exif === 'loading' ? (
            <p className="text-xs text-slate-400 dark:text-slate-500 italic">Analysing…</p>
          ) : exif === null ? (
            <p className="text-xs text-slate-500 dark:text-slate-400">No EXIF metadata found.</p>
          ) : (
            <dl className="space-y-1">
              {(exif.make || exif.model) && (
                <Row icon="📷" label="Device" value={[exif.make, exif.model].filter(Boolean).join(' · ')} />
              )}
              {exif.dateTaken && (
              ...
              {exif.software && (
                <Row icon="💻" label="Software" value={exif.software} />
              )}
              {exif.orientation && exif.orientation !== 1 && (
                <Row icon="🔄" label="Orientation" value={`${exif.orientation} (auto-corrected)`} />
              )}
            </dl>
          )}
            ...
            <div className="mt-2 pt-2 border-t border-slate-100 dark:border-slate-700">
              {hasGps && (
                <p className="text-xs text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 rounded px-2 py-1 mb-2 border border-amber-100 dark:border-amber-800/30">
                  ⚠ This photo contains your GPS location. Remove it before sharing.
                </p>
              )}
              <button
                type="button"
